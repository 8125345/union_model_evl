...
def dict_to_array(d):
    return np.frombuffer(d['raw'], dtype=d['dtype']).reshape(d['shape'])
...
class Transcriber:
  def __init__(self, id=None):
    ...
    self.busy = False
    self.log('Transcriber init')
    self.queue_key = 'spec-queue'
    self.task_id = None
    self.busy = False
  ...
  def gen_input(self):
    ...
    while True:
      ...
      if task is None:
          ...
          self.task_id, timestamp = task['task_id'], task['timestamp']
          if time.time() - timestamp > server_config.timeout - 1: # 1 sec for processing
              self.log('task timeout, droped')
              continue

          meta = task
          r.set(self.task_id, pickle.dumps(meta), ex=60)

          self.log('task get {}'.format(self.task_id))
          self.log('start preprocess')
          spec = dict_to_array(payload['spec'])
  ...
  def pianoroll_to_note_sequence(self,
    ...
    # total_frames += 1
      ...
      if self.task_id:
        # for debugging
        # midi_io.sequence_proto_to_midi_file(sequence, '/tmp/{}.midi'.format(self.task_id))
        self.enqueue_result(sequence, chunk)


  def _run(self):
    with self.sess.graph.as_default():
      self.log('Transcriber Start')
  ...
  def enqueue_result(self, sequence, chunk):
    ...
    result = pickle.loads(r.get(self.task_id))
    result.update({
        'result' : {
            'midi': midi_b64,
            'onsets': array_to_dict(chunk.onset_probs_flat),
            'sequence': MessageToDict(sequence),
        },
    })
    data = pickle.dumps(result)
    r.publish('transcribe-result', data)
