...
from deepiano.music import melspec_input
...
def wav_to_spec(wav_audio, hparams):
  ...
  if hparams.spec_type == 'raw':
    ...
    if hparams.spec_log_amplitude:
      spec = librosa.power_to_db(spec, top_db=None)
...
def provide_single(example, hparams, is_training):
  """Process an Example proto into a model input."""
  input_tensors = preprocess_example(example, hparams=hparams, is_training=is_training)
  features, labels = input_tensors_to_model_input(input_tensors, hparams, is_training)

  features = FeatureTensors(
    spec=tf.expand_dims(features.spec, 0),
    length=tf.expand_dims(features.length, 0),
    sequence_id=tf.expand_dims(features.sequence_id, 0),
    spectrogram_hash=tf.expand_dims(features.spectrogram_hash, 0))

  labels = LabelTensors(
    labels=tf.expand_dims(labels.labels, 0),
    label_weights=tf.expand_dims(labels.label_weights, 0),
    onsets=tf.expand_dims(labels.onsets, 0),
    offsets=tf.expand_dims(labels.offsets, 0),
    velocities=tf.expand_dims(labels.velocities, 0),
    note_sequence=tf.expand_dims(labels.note_sequence, 0))
