...
def process_record(wav_data,
  ...
  try:
    if sample_rate is None:
      samples, sample_rate = audio_io.wav_data_to_samples_detect_sr(wav_data)
    else:
  ...
  total_time = ns.total_time if ns.total_time != 0 else len(samples) / sample_rate

  samples = librosa.util.normalize(samples, norm=np.inf)
  if max_length == min_length:
    splits = np.arange(0, total_time, max_length)
  elif max_length > 0:
  ...
  for start, end in zip(splits[:-1], splits[1:]):
    if end - start + 1E-9 < min_length:
      continue

    if (start == 0 and end == total_time) or ns.total_time == 0:
      new_ns = ns
    else:
